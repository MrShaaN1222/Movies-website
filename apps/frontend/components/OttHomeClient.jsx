...
function getPreviewUrl(item) {
  return item?.trailerHlsUrl || item?.previewHlsUrl || item?.hlsPreviewUrl || item?.hlsUrl || "";
}
      ...
      {isHovered ? (
        <div className="absolute left-1/2 top-0 z-30 w-[17rem] -translate-x-1/2 -translate-y-2 overflow-hidden rounded-xl border border-white/15 bg-slate-900 shadow-2xl shadow-black/80 group-first:left-0 group-first:translate-x-0 group-last:left-auto group-last:right-0 group-last:translate-x-0">
          <div className="relative aspect-video w-full bg-black">
            ...
            {previewUrl ? (
              <button
                ...
                aria-label={previewMuted ? `Unmute preview for ${item.title}` : `Mute preview for ${item.title}`}
          ...
          <div className="space-y-2 bg-slate-900 px-3 py-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  aria-label={inWatchlist ? `Remove ${item.title} from wishlist` : `Add ${item.title} to wishlist`}
                  onClick={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    onToggleWatchlist(item.slug);
            ...
            <p className="line-clamp-2 text-base font-semibold leading-tight text-white">{item.title}</p>
            <p className="text-xs capitalize text-zinc-300">{String(item.type || "").replace(/-/g, " ")}</p>
            {item.contentRating ? (
...
export default function OttHomeClient({ items = [], view = "all", section = "" }) {
  ...
  const featuredPreviewUrl = getPreviewUrl(featured);
